...
from ...constants import SCHEMA_TEST_PROMOTE, SCHEMA_PROD_PROMOTE, BATCHTIME, PREV_BATCHTIME, DAILY_BATCHTIME
...
class CopySQL(Step):
    ...
    def __str__(self):
        return f"Copy to SQL table {self.prod_location}"
    ...
    def _quality_check(self, test):
        ...
        try:
            ...
            raise RuntimeError(f"The table {self.location} wasn't properly created by this Copy to SQL step")
        if self.columns == '*':
...
""")
        if not debug:
            query = f"""
                    ...
                    create table {self.temporary_table} as select * from
                    {data_table};
                    """
            print(query)
            execute(query)
        ...
        if not debug:
            ...
            if (self.truncate_table or self.append_to_table) and table_exists:
                ...
                copy_query = f"""
                    ...
                    create table {self.location} as select * from {self.temporary_table};
                """
            query += copy_query
            print(query)
            execute(query)
            ...
            ret = getattr(self, "sql_retention", None)
            if ret:
                ...
                for t in cleanup_tables:
                    for c in [PREV_BATCHTIME, BATCHTIME]:
                        if c in t:
                            ...
                            if t_older not in getattr(self, "sql_cleanup_exceptions", []):
